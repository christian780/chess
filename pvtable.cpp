...
int GetPvLine(const int depth, S_BOARD *pos) {
    ...
    ASSERT(depth < Constants::MAXDEPTH)
    while(move != NOMOVE && count < depth) {
        ASSERT(count < Constants::MAXDEPTH);
...
void StoreHashEntry(S_BOARD *pos, const int move, int score, const int flags, const int depth) {
    ...
    ASSERT(depth>=1&&depth<Constants::MAXDEPTH);
    ASSERT(flags>=HFALPHA&&flags<=HFEXACT);
    ASSERT(score>=-INF&&score<=INF);
    ASSERT(pos->ply>=0&&pos->ply<Constants::MAXDEPTH);
        ...
        *move = pos->HashTable->pTable[index].move;
        if(pos->HashTable->pTable[index].depth >= depth){
            ...
            *score = pos->HashTable->pTable[index].score;
            ...
            case Constants::Enums::HFALPHA: if(*score<=alpha) {
                    *score=alpha;
                    return TRUE;
                }
                break;
            case Constants::Enums::HFBETA: if(*score>=beta) {
                    *score=beta;
                    return TRUE;
                }
                break;
            case Constants::Enums::HFEXACT:
                return TRUE;
                break;
